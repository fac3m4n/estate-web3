...
contract Marketplace is Ownable, ReentrancyGuard, Pausable, AccessControl, IERC721Receiver, Initializable {
    ...
    function buyProperty(uint256 _tokenId, uint256 _price) external whenNotPaused {
        ...
        if (isTax) {
            ...
            TUSDtoken.transferFrom(msg.sender, taxReceiver, amountToTaxReceiver);
            TUSDtoken.transferFrom(msg.sender, listing.seller, amountToSeller);
        } else {
            TUSDtoken.transferFrom(msg.sender, taxReceiver, listing.price);
        }

        if (listing.highestBidder != address(0)) {
            TUSDtoken.transfer(listing.highestBidder, listing.highestBid);
        }
    ...
    function listProperty(uint256 _tokenId, uint256 _price, bool _canBid) external whenNotPaused {
        require(_price < TUSDtoken.totalSupply(), "listProperty: The price cannot exceed USD total supply");
    ...
    function createProperty(address lister, uint256 _tokenId, uint256 _price, bool _canBid) external whenNotPaused {
        ...
        //require(_price<=USDtoken.totalSupply(), "CreateProperty: The price cannot exceed USD total supply");
    ...
    function updatePrice(uint256 _tokenId, uint256 _price) external whenNotPaused {
        MarketItem storage listing = items[_tokenId];
        require(_price < TUSDtoken.totalSupply(), "updatePrice: Price can not exceed USD total supply");
    ...
    function placeBid(uint256 _tokenId, uint256 bidAmount) external whenNotPaused {
        ...
        TUSDtoken.transferFrom(msg.sender, address(this), bidAmount);
    ...
    function acceptBid(uint256 _tokenId) external whenNotPaused {
        ...
        if (isTax) {
            ...
            TUSDtoken.transfer(taxReceiver, amountToTaxReceiver);
            TUSDtoken.transfer(msg.sender, amountToSeller);
        } else {
            TUSDtoken.transfer(msg.sender, listing.highestBid);
        }
    ...
    function buyWithInstallment(uint256 _tokenId, uint256 _price) external whenNotPaused {
        ...
        require(TUSDtoken.balanceOf(msg.sender) >= downPayment, "makePayment: Your tUSD balance is insufficent");
        ...
        if (isTax) {
            ...
            TUSDtoken.transferFrom(msg.sender, listing.seller, downPayment);
        }
    ...
    function makePayment(uint256 _tokenId) external whenNotPaused nonReentrant {
        ...
        require(
            TUSDtoken.balanceOf(msg.sender) >= installmentInfo.monthlyAmount,
        ...
        TUSDtoken.transferFrom(msg.sender, listing.seller, installmentInfo.monthlyAmount);
    ...
    // Set USD token
    function setUSD(IERC20 _token) external onlyOwner {
